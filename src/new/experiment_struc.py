...
def proc_sum_desc_vec(file_name):
    ...
    with open(file_name+'_proc.csv', newline='') as csvfile:
        ...
        for row in reader:
            ...
            dic = term_count(text)
            terms = dic.most_common()
            output = str(row['component'] in (None, '') and 'null' or row['component']) + ','
...
def proc_sum_desc(file_name):
    with open('../data/'+file_name+'.csv', newline='') as csvfile:
        reader = csv.DictReader(csvfile)
        print('issue_id,component,reporter,summary,description,summary_proc,description_proc,'+intent)
        for row in reader:
...
def doExperiment():
    ...
    X = np.array(chou_data[data],dtype=object)
    y = np.array(chou_data[target],dtype=object)
    ...
    for k in range(10):
        ...
        X_train = np.array(np.concatenate(X_train),dtype=object)
        y_train = list(y_folds)
        y_test = y_train.pop(k)
        y_train = np.array(np.concatenate(y_train),dtype=object)
